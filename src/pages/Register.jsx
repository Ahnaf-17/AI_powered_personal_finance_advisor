  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-violet-700 via-indigo-600 to-indigo-700 flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="absolute -bottom-24 -left-24 w-80 h-80 bg-indigo-500/20 rounded-full blur-3xl"></div>
        ...
        <div className="relative space-y-6">
          ...
          <p className="text-indigo-200 text-lg leading-relaxed">
            Join thousands of people who improved their financial health with smart tracking.
          </p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12 bg-white">
        <div className="w-full max-w-sm space-y-8">
              ...
              {password.length > 0 && (
                <div className="flex items-center gap-2 mt-1.5">
                  <div className="flex gap-1 flex-1">
                    {[1,2,3].map((i) => (
                      <div key={i} className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${i <= strength ? strengthColor : "bg-slate-100"}`}></div>
                    ))}
                  </div>
                  <span className={`text-xs font-medium ${["","text-red-500","text-amber-500","text-emerald-600"][strength]}`}>{strengthLabel}</span>
                </div>
              )}
            </div>
          ...
          <p className="text-center text-sm text-slate-500">
            ...
            <Link to="/login" className="text-indigo-600 font-semibold hover:text-indigo-500 transition-colors">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
